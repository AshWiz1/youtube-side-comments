...
YouTube puts the comments below the video, so you scroll down to read them and the video scrolls away. This moves them into a resizable column on the right, in the space the recommendations were wasting, and drops the recommendations into a full-width grid underneath.
...
**Comments beside the video.** Comments move into a column on the right, with their own scrollbar. Reading the thread never moves the video. These are YouTube's own comments, not a rebuild, so sorting, replies, likes and the comment box all work exactly as they did.
...
**1.** Download [youtube-side-comments-0.1.0.zip](https://github.com/AshWiz1/youtube-side-comments/releases/download/v0.1.0/youtube-side-comments-0.1.0.zip) from the [latest release](../../releases/latest). It's about 44 KB.

**2.** Unzip it somewhere it can stay. You get a folder called `youtube-side-comments`. Don't move or delete it afterwards, because Chrome loads the extension *from that folder*, so if the folder goes, the extension does too.
...
```
npm test                          # everything; requires Chrome and a network
...
npm run package                   # build dist/, the zip that goes on a release
```
...
`tools/screenshots.mjs` regenerates the images in this README from a live browser, and `npm run package` builds the release zip.
