...
- [x] The Strip lays out four tiles per row at a wide viewport.
- [x] Tiles are substantially larger than the current 359×134.
- [x] Tile proportions look right at the new size — neither squat nor stretched.
- [x] The Strip remains responsive at narrower viewports rather than holding four columns into a width that cannot take them.
- [x] No horizontal overflow at any width.
- [x] The Strip's Step Aside revert — returning the related list to its exact original position — is unregressed.
- [x] Whether the home-page tile shape was reachable is stated explicitly in the report.
...
- [x] Shorts in the Strip do not each occupy a row of their own.
- [x] A Short's tile holds a sensible proportion rather than being stretched to the row's height.
- [x] A row holding both Shorts and ordinary videos lays out coherently rather than one shape dictating the other.
- [x] The fix does not simply hide Shorts — they are legitimate recommendations.
